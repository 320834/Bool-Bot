...
async def generate_meme_video(ctx, text, file_id, file_name):
    ...
    video = VideoFileClip(google_drive_feat.temp_dir +
                          video_name).set_position("bottom")
    ...
    background = create_white_background_clip(
        video_height, video_width, line_height, len(lines), time)
    ...
    result.write_videofile(google_drive_feat.temp_dir +
                           "temp.mp4", threads=4, fps=20, logger=None)
...
def determine_lines(text, width, fontsize):
    ...
    for index, char in enumerate(text):
        if(
            ...
            # Ensure characters in the beginning are in the first line
            index >= char_limit-BUFFER and
            # Ensure current line is atleast 2 twice the length of the buffer
            index-last_line_index > 2*BUFFER and
            (
                # Ensure character index falls between char_limit and the char_limit - buffer
                index % char_limit >= char_limit - BUFFER or
...
def create_white_background_clip(video_height, video_width, line_height, num_of_lines, time):
    ...
    text_height = video_height + Y_OFFSET + line_height*num_of_lines
